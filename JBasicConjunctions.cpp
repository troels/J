...
namespace J {
  shared_ptr<JWord> RankConjunction::operator()(shared_ptr<JWord> lword, shared_ptr<JWord> rword) const {
	...
	(array.get_rank() == 1 && array.get_dims()[0] == 1)) {
      return shared_ptr<JWord>(new RankVerb(get_machine(), verb, *ptr, *ptr, *ptr));
    } else if (array.get_dims()[0] == 2) {
      return shared_ptr<JWord>(new RankVerb(get_machine(), verb, *ptr, *(ptr + 1), *(ptr + 1)));
    } else if (array.get_dims()[0] == 3) {
      return shared_ptr<JWord>(new RankVerb(get_machine(), verb, *ptr, *(ptr + 1), *(ptr + 2)));
    }
